...
from musikku.config import BOT_NAME as bn
from musikku.config import DURATION_LIMIT
from musikku.config import UPDATES_CHANNEL as updateschannel
from musikku.config import que
from musikku.function.admins import admins as a
from musikku.helpers.admins import get_administrators
from musikku.helpers.channelmusic import get_chat_id
from musikku.helpers.decorators import authorized_users_only
from musikku.helpers.filters import command, other_filters
from musikku.helpers.gets import get_file_name
from musikku.services.callsmusic import callsmusic, queues
from musikku.services.callsmusic.callsmusic import client as USER
from musikku.services.converter.converter import convert
from musikku.services.downloaders import youtube
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                ...
                [InlineKeyboardButton(text="Support Channel", url="https://t.me/levinachannel")],
...
async def deezer(client: Client, message_: Message):
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 Kesalahan permintaan banjir 🔴 \nUser {user.first_name} Sudo kagak sabi gabung."
                        "\n\nAtau tambahin secara manual</b>",
                    )
    try:
        await USER.get_chat(chid)
        # lmoa = await client.get_chat_member(chid,wew)
        ...
        await lel.edit(
            f"<i> {user.first_name} Kagak sabi gabung cuk, coba admin play dulu atau tambahin {user.first_name} manual</i>"
        )
        return
    ...
    queryy = text[1]
    res = lel
    await res.edit(f"Mencari untuk `{queryy}` di deezer")
    try:
        ...
        await res.edit("Tidak Menemukan Apa-apa, Coba pake bahasa inggris!")
        return
    keyboard = InlineKeyboardMarkup(
        [
            [
            ...
            [InlineKeyboardButton(text="Support Channel", url=f"Https://t.me/levinachannel")],
            [InlineKeyboardButton(text="❌ Close", callback_data="cls")],
        ]
    )
    file_path = await convert(wget.download(url))
    await res.edit("Membuat Gambar Mini")
    await generate_cover(requested_by, title, artist, duration, thumbnail)
    chat_id = get_chat_id(message_.chat)
    if chat_id in callsmusic.pytgcalls.active_calls:
        await res.edit("menambahkan dalam antrian")
        position = await queues.put(chat_id, file=file_path)
        qeue = que.get(chat_id)
        s_name = title
        ...
        appendable = [s_name, r_by, loc]
        qeue.append(appendable)
        await res.edit_text(f"#️⃣ Antri di posisi {position}")
    else:
        ...
        try:
            ...
            res.edit("Panggilan grup tidak terhubung karena saya tidak dapat bergabung")
            return
...
async def jiosaavn(client: Client, message_: Message):
        ...
        for administrator in administrators:
            if administrator == message_.from_user.id:
                ...
                try:
                    ...
                    # print(e)
                    await lel.edit(
                        f"<b>🔴 Permintaan banjir 🔴 \nUser {user.first_name} tidak dapat bergabung dengan grup Anda karena banyaknya permintaan untuk userbot! Pastikan pengguna tidak dilarang di grup."
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        m = await client.send_photo(
            ...
            reply_markup=keyboard,
            photo="final.png",
            caption=f"#️⃣ Antri di posisi {position}",
        )
